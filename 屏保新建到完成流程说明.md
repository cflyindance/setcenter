...
| 路径 | 适用场景 | 摘要 |
|------|----------|------|
| **A. 先保存，后下发** | 先落库主题，稍后再选门店下发 | `新建屏保` → **保存** → `屏保列表` → 该主题 **⋯ → 下发门店** → 下发四步 → **完成** |
| **B. 保存并立即下发** | 创建完直接进入下发向导 | `新建屏保` → **下发门店** →（保存主题后）自动进入下发四步 → **完成** |
...
下发子流程共 **4 步**，顶栏进度条宽度与页面大致对应：

| 顺序 | 页面文件 | 标题（顶栏） | 进度条（原型） | 主要操作 |
|:----:|----------|--------------|----------------|----------|
| 1 | `store-select.html` | 选择门店 | 25% | 勾选至少一家门店 → **Next** |
| 2 | `channel-select.html` | 选择渠道 | 50% | 至少勾选一个渠道（可全选）→ **Next** |
| 3 | `distribute-time.html` | 选择下发时间 | 75% | 自定义或「立即」下发时间 → **Next** |
| 4 | `effective-time.html` | 选择展示生效时间 | 100% | 自定义或「立即」生效；点 **完成** → 二次确认 → 写回主题并跳转列表 |
...
| **下一步** | `distribute-time.html` |

### 3.5 选择下发时间

| 文件 | `distribute-time.html` |
|------|-------------------------|
| **操作** | **自定义**：日期 + 时间；**立即**：使用当前日期时间写入。 |
| **写出** | `distribute_date`（`YYYY-MM-DD`）、`distribute_time`（`HH:mm`）。 |
...
|------|-------------------------|
| **操作** | **自定义**：开始/结束 日期+时间；**立即**：对应主题为「立即生效」逻辑（见状态说明文档）。顶栏 **完成** → 弹出 **确认下发** → **确定** 后执行写库。 |
| **校验（点「完成」及确认时）** | 自定义时：生效开始 **不得早于** 下发时间；结束 **不得早于** 开始；**同渠道多主题生效时段不可重叠**（与 `kiosk_themes` 中其他主题比较，排除当前 `current_theme_id_for_distribute`）。 |
| **写回主题**（`executeSubmitDistribute`） | 对 `current_theme_id_for_distribute` 对应主题更新：`distributeTime`、`effectTime`、`distributeChannels`、`distributedStores`（含门店状态、渠道状态、`shelfStatus` 等模拟结果）、`localStorage.kiosk_themes` 整体保存。 |
| **清理** | 移除 `selected_stores_for_distribute`、`selected_channels_for_distribute`、`current_theme_id_for_distribute` 等（以代码为准）。 |
...
| `distribute_date` | `distribute-time` | 下发日期 `YYYY-MM-DD` |
| `distribute_time` | `distribute-time` | 下发时间 `HH:mm` |

完成下发后，与本次流程相关的临时键会在 `effective-time.html` 中清除（避免污染下一次下发）。
...
flowchart LR
  ...
  subgraph distribute [下发四步]
    S[store-select.html]
    H[channel-select.html]
    D[distribute-time.html]
...
```
store-select.html → channel-select.html → distribute-time.html → effective-time.html → kiosk-theme-list.html
```
...
下发完成后返回 **`kiosk-theme-list.html`**，主题已具备下发时间、生效时间、渠道与门店结果等字段。后续可：

- 查看 **下发状态 / 生效状态**（计算规则见 [`屏保状态与操作说明.md`](屏保状态与操作说明.md)）；  
...
| `distribute-time.html` | 下发第 3 步 |
| `effective-time.html` | 下发第 4 步与完成写库 |
