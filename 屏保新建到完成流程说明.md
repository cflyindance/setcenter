...
| **默认时间（历史）** | 若主题存在 **`lastDistributePrefs`** 且含 `startDate` / `startTime` / `endDate` / `endTime`，则优先填入四个控件；否则用页面原默认（当前时刻等）。 |
| **校验（点「完成」及确认时）** | 四项均需填写；结束 **不得早于** 开始；**同渠道多主题生效时段不可重叠**（与 `kiosk_themes` 中其他主题比较，排除当前 `current_theme_id_for_distribute`）。 |
| **写回主题**（`executeSubmitDistribute`） | 对 `current_theme_id_for_distribute` 对应主题更新：`distributeTime` 固定为 `'--'`、`effectTime`、`distributeChannels`、`distributedStores`（含门店状态、各渠道 `channelStatuses` 等）；并写入 **`lastDistributePrefs`**（`stores`、`channels`、四个时间字段）供下次下发恢复；`localStorage.kiosk_themes` 整体保存。 |
| **清理** | 移除 `selected_stores_for_distribute`、`selected_channels_for_distribute`、`current_theme_id_for_distribute` 及历史键 `distribute_date` / `distribute_time`（若存在）等。**`lastDistributePrefs` 保留在主题上，不清理**（屏保下架不影响该快照）。 |
